...
const PropertyCard = ({ property }) => {
  ...
  return (
    ...
    <div className="p-4">
      ...
      >
          ${getRateDisplay[`${rates.monthly ? 'monthly' : rates.weekly ? 'weekly' : 'nightly'}`]}
      </h3>

      <div className="flex justify-center gap-4 text-gray-500 mb-4">
        <p>
          <FaBed className='inline mr-2'/>{beds}{' '}
            <span className="md:hidden lg:inline">Bed{`${beds > 1 ? 's' : ''}`}</span>
        </p>
        <p>
          <FaBath className='inline mr-2' /> {baths}{' '}
            <span className="md:hidden lg:inline">Bath{`${baths > 1 ?'s' : ''}`}</span>
        </p>
        <p>
